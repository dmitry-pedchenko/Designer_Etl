...
def create_config(path_config, obj):
    ...
    et.SubElement(root, 'dbtype').text = f'{obj.pref["dbtype"]}'
    et.SubElement(root, 'dbHost').text = f'{obj.pref["dbHost"]}'
    et.SubElement(root, 'dbSchema').text = f'{obj.pref["dbSchema"]}'
    et.SubElement(root, 'dbUser').text = f'{obj.pref["dbUser"]}'
    et.SubElement(root, 'dbPass').text = f'{obj.pref["dbPass"]}'
    et.SubElement(root, 'dbBase').text = f'{obj.pref["dbBase"]}'
    et.SubElement(root, 'dbPort').text = f'{obj.pref["dbPort"]}'
    et.SubElement(root, 'loadMode').text = f'{obj.pref["Load Mode"]}'.lower()
    et.SubElement(root, 'dict').text = f'{obj.pref["checkBox_Dictionary"]}'.lower()
    et.SubElement(root, 'checkMode').text = f'{obj.pref["checkBox_checkMode"]}'.lower()
    et.SubElement(importXml, 'path').text = f'{obj.pref["excelFileName"]}'
    et.SubElement(importXml, 'sheetNumber').text = f'{obj.pref["comboBox_list_source_excel"] + 1}'

    linkedColumns.attrib['mode'] = f'{obj.pref["checkBox_checkMode"]}'.lower()

    for column in obj.list_of_source_cols_links:
        ...
        if column['filter_box'].checkBox_widget_for_filter_check.isChecked():
            ...
            if column['filter_box'].combo_box_filter_condition.currentText() == '---':
                dial_win = QtWidgets.QDialog(obj)
            ...
            if col_type == 'str':
                ...
                obj.loggerInst.raiseError(0, 'Unknown filter type')
    ...
    if obj.pref_gui.ui.checkBox_checkMode.isChecked():
        pathToLinkFile = et.SubElement(linkedColumns, 'pathToLinkFile')
        pathToLinkFile.text = f"{obj.pref_gui.ui.compare_file.text()}"
        linkedFileSheetNumber = et.SubElement(linkedColumns, 'linkedFileSheetNumber')
        linkedFileSheetNumber.text = f"{obj.pref_gui.comboBox_set_list_checked.currentIndex() + 1}"
        both = et.SubElement(linkedColumns, 'both')
        both.text = f"{obj.pref_gui.ui.checkBox_both.isChecked()}".lower()

        for col in obj.pref['col_to_check']:
    ...
    if obj.pref_gui.ui.checkBox_Dictionary.isChecked():
        tables = et.SubElement(withDict, 'tables')
        withDict.attrib['mode'] = 'true'

        for table_count in obj.list_of_dict_pref:
    ...
    path.text = f"{obj.pref['target_table_name']}"

    for col in obj.list_of_receiver_cols_links:
