...
def create_dict_column(pref, parent, cur_column_pref):
    ...
    for row in cur_column_pref['arrOfDictColumns']:
        colNameRow = ColumnNameRow(row, main_row)
        ...
        replaceRow = ReplaceRow(row, parent, colNameRow, after_widget=addValueBothRow)

    parent.addTopLevelItem(main_row)
    parent.addTopLevelItem(dict_pref_indxDbColumn)
    parent.addTopLevelItem(dict_pref_indxColumnDic)
    parent.addTopLevelItem(colNameRow)
    parent.addTopLevelItem(colNameDbRow)
    parent.addTopLevelItem(colTypeRow)
    parent.addTopLevelItem(cropEndRow)
    parent.addTopLevelItem(addValueEndRow)
    parent.addTopLevelItem(takeFromBeginRow)
    parent.addTopLevelItem(cropBeginRow)
    parent.addTopLevelItem(addValueBeginRow)
    parent.addTopLevelItem(addValueBothRow)
    parent.addTopLevelItem(replaceRow)
...
class MainDictTableName(QtWidgets.QTreeWidgetItem):
    def __init__(self, cur_column_pref, parent):
        ...
        combo_box_dictTableName = QtWidgets.QComboBox()
        combo_box_dictTableName.addItems(list_combo_box_dictTableName)
        combo_box_dictTableName.setCurrentIndex(list_combo_box_dictTableName.index(cur_column_pref['dictTableName']))
        parent.setItemWidget(self, 1, combo_box_dictTableName)
...
class ColumnNameRow(QtWidgets.QTreeWidgetItem):
    def __init__(self, column_property, parent):
