...
14. [Редактирование конфигурации загрузки](#Редактирование-конфигурации-загрузки)
...
Перед загрузкой в таблицу `Table` выполним тестовый запуск чтобы убедиться в том, что сформировались верные запросы к базе

<image src="readme/Load/Table_test.gif">

Как мы видим, система автоматически определила значения для полей `id_1` и `id_2`.
...
Об успешном закрытии проекта можно понять по отсутствию имени проекта в имени окна в осном окне программы.
 ...
 [:arrow_up:Оглавление](#Оглавление)
## Редактирование конфигурации загрузки

Далее продемонстрируем пример того, что при неверных параметрах подключения к базе данных в файле конфигурации система
не сможет открыть файл конфигурации. Но чтобы не создавать файл заново и вводить верные параметры подключения существует
функция `DB conn editor` которая открывает файл конфигурации без подключения к базе данных, и в ней мы можем отредактировать
параметры конфигурации и сохранить их.

Продемонстрируем это. Сначала введём заведомо ложные параметры подключения:

<image src="readme/edit/table1_edit_con_for_err.gif">

Затем попробуем подключиться к базе данных и увидим то, что это уже не получается. И затем при помощи раздела `DB conn editor`
мы сможем отредактировать данные и снова открыть файл конфигурации.

 <image src="readme/edit/table1_edit2.gif">
 
 Далее продемнострируем некоторые возможности системы загрузки.
 Допустим, на нужно загружать в таблицу `Table_1` в поле `val_1` значения из полей `dict_1_1` и `dict_1_2`. Причем нужно
 после значений из поля `dict_1_1` нужно поставить знак `_` а после значений из `dict_1_2` поставить `.`
 При этом начмнужно заменить значения из поля `dict_1_2`: при значении в ячейке `test_4` нужно заменить его на `test_four`
  а при значении`test_5` заменить на `test_five`. Так же отфильтруем данные по дате: в полсе `val_date` мы будем брать
  значения только те, которые идут после `2020.01.01` . В поле `Table_1.val_2` данные будут идти без изменения из поля `val_2`
  
Приступим:

 <image src="readme/edit/table1_show_con_edit.gif">
 
 Отметим то, что при загрузке в одно поле из нескольких полей нужно поставить галочку в чекбокс `Составное` в настройках полей базы данных
 Если необходимо изменить последовательность конкатенации строк из источника, необходимо поменять последовательность их создания в
 настройках полей источника (какая строка поля источника выше, то и будет раньше суммироваться)
 
 Теперь запустим на тестирование отредактированный файл конфигурации и посмотрим какие запросы формируются:
 
  <image src="readme/Load/table1_show_load.gif">
  
Как видно из составленных запросов: несоклько строк выпало из запросов. Это те строки у которых дата в поле `dict_1_3` меньше
даты указанной в поле фильтрации. А также мы видим то что теперь в поле `val_1` идут данные из полей `dict_1_1` и `dict_1_2` и с добавленными
символами после данных. Так же видно то, что значение `test_5` изменилось на `test_five` (`test_4` не попало в выборку так как
дата в поле `dict_1_3` у него меньше). Отмечу то, что первой операцией из редактирования данных в поле источника идет замена и только потом
все остальные (сначала заменилось поле на `test_five` и только потом добавился символ `.`) 